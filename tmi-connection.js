...
var checkCommands=function(message){
	switch(message.message){
		case "!poyo":
			...
			$("#messages").prepend("<div><strong>"+message.tags.displayName+":</strong> "+"poyoyoyoyy?????"+"</div>");
		break;
		case "!help": case "!h":
			alert("help!");
		break;
...
{id:"6b0e154d-fc18-4983-a2cd-088a2194f805",process:testEvent}
...
function addListeners() {
  ...
  function handleMessage(channel, userstate, message, fromSelf) {
    ...
    userstate.name = name;
    showMessage({ chan, type: 'chat', message, data: userstate , timeout:20000});
  }
